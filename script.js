...
// ---------------------------------------------
// 2) Load CSV from Google Sheets (TIMESTAMP FIXED)
// ---------------------------------------------
...
// ---------------------------------------------
// 6) CHART FUNCTIONS (unchanged - all 5 charts)
// ---------------------------------------------
function renderLOBChart(data) {
    ...
    lobChart = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { ticks: { autoSkip: false, maxRotation: 45, minRotation: 0, color: "#9ca3af" }, grid: { display: false } },
                y: { beginAtZero: true, ticks: { color: "#9ca3af" }, grid: { color: "rgba(31, 41, 55, 0.7)" } }
            }
        }
    });
}
...
function buildDailyAgg(rows) {
    ...
    rows.forEach(r => {
        ...
        if (!map[key]) {
            map[key] = {
                offered: 0, answered: 0, abandoned: 0, ansUnder30: 0,
                flowOuts: 0, talkTime: 0, holdTime: 0, acwTime: 0, handleTime: 0
            };
        }
    ...
    return days.map(date => {
        ...
        return { date, offered: b.offered, answered: b.answered, abandoned: b.abandoned, slPct, abnPct, ahtSec };
    });
}
...
function renderVolumeTrendChart(rows) {
    ...
    volumeTrendChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                { label: "Offered", data: daily.map(d => d.offered), borderColor: "rgba(59, 130, 246, 1)", backgroundColor: "rgba(59, 130, 246, 0.2)", tension: 0.25, fill: true },
                { label: "Answered", data: daily.map(d => d.answered), borderColor: "rgba(16, 185, 129, 1)", backgroundColor: "rgba(16, 185, 129, 0.15)", tension:
